...
try {
    ...
    // $meetingsql="INSERT into interview(id,userid,employerid,jobpostid,jobapplyid,meetingid,time,url,password) values('$id','$userid','$employerid','$jobpostid','$jobapplyid','$meeting_id','$meeting_time','$meeting_link','$meeting_password')";
    // if(mysqli_query($connection,$meetingsql)){
    //     $updatesql="UPDATE jobapply set status='called for interview' where id='$jobapplyid'";
    //     if(mysqli_query($connection,$updatesql)){
    //         header("Location: employer_viewjobapplication.php?interview_msg=Applicant called for interview ");
    //         exit();
